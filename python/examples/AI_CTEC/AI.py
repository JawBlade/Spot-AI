...
from helpers import adjust_volume, load_on_spot, play_on_spot, delete_from_spot, record
...
async def main():
    ...
    logger.info(f"Final text to speak: {text_to_speak}")

    try:
        generate_final_wav(text_to_speak, TTS_RAW_FILE, FINAL_WAV_FILE)
    except Exception as e:
        logger.error(f"Error generating WAV file: {e}")
        return

    logger.info(f"Final WAV file ready: {FINAL_WAV_FILE}")
